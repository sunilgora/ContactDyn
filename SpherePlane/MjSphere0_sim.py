...
def simulate(dampratio, timeconst):
...
plt.subplots_adjust(bottom=0.35, top=0.95, left=0.175, right=0.9)
...
# Slider for time constant (first component of solref)
ax_slider_time = plt.axes([0.35, 0.15, 0.5, 0.03])
slider_time = Slider(ax_slider_time, 'Time Constant', 0.001, 0.1, valinit=0.02, valstep=0.001)

# Slider for damping ratio (second component of solref)
ax_slider_damp = plt.axes([0.35, 0.1, 0.5, 0.03])
slider_damp = Slider(ax_slider_damp, 'Damping Ratio', 0.01, 2.0, valinit=1.0)
...
def plot_func(event):
    timeconst = slider_time.val
    dampratio = slider_damp.val
    deps, fors = simulate(dampratio, timeconst)
    ax.plot(deps, fors, label=f'solref: [{timeconst:.3f}, {dampratio:.2f}]', linewidth=2)
    ax.legend(loc='upper right')
    plt.draw()
    #save figure
    #Adjust margins and save
    fig.savefig(f'MjSphere0.pdf', dpi=300)

button.on_clicked(plot_func)
# print('Modified stiffness:',1/4*(1-0.475)*9.81*(0.95)**2/(0.006*0.475**2))
plt.show()